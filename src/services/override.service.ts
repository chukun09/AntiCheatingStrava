...
export async function overrideActivityStatus(
  ...
  const flagReason = isLegit 
    ? (reason || 'Duyệt thủ công bởi Ban Tổ Chức')
    : (reason || 'Bị từ chối / Hủy thủ công bởi Ban Tổ Chức');
  ...
  // 2. Recalculate user stats to ensure 100% mathematical data consistency
  const stats = await recalculateUserStats(existingActivity.userId);
